...
$result = "SELECT  SEC_TO_TIME( SUM( TIME_TO_SEC( `query_time` ) ) ) AS total_time  
FROM user_query_log"; 
$conn->query($result);

if(!$conn->query($result)){
    echo "summation Failed: (" . $conn->errno . ") " . $conn->error;
}
else {
echo"summation success";
...
// set few params
$p->data =array(array(3,7,9,1,4,6,8,2,99),array(5,3,8,2,6,2,9,2,6));
$p->chart_type = "area";
