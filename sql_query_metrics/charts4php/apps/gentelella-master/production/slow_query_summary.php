...
for($x= 1;$x<=2;$x++){
    ...
    /*****************avg execution time*****************/
    $sql_avg="SELECT SEC_TO_TIME(AVG(TIME_TO_SEC(`query_time`))) AS query_time FROM user_slow_log where start_time ='$query_date'";
  ...
  $sql_sum="SELECT SEC_TO_TIME(Sum(TIME_TO_SEC(`query_time`))) AS query_time FROM user_slow_log where start_time ='$query_date'";
...
$queryInsertQueryInfo = "INSERT INTO  slow_queries_summary(start_time,q_num,avg_exe_time,tot_exe_time) VALUES ('$query_date','$tot_row','$avg','$total');";
